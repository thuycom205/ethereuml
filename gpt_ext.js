...
function  xvSum(parent) {
    ...
    ldid = $('.sum-content').last().attr('data-id');
    if (xf == ldid) {
        $('body').attr('xf', '100');
    }
    var pe = $('.sum-content').filter(function () {
        return $(this).attr('data-id') == xf
    })
    var html = pe.html();
    var tag = pe.attr('data-tag');
    ...
    parent.cfm= parent.cfm + 1;

    if (parent.cfm == parent.max) {
        //sumit the last content
        console.log('max reach in xvSum');
        var content = jQuery('.markdown').last().html();
        console.log(content);
        jQuery.ajax({
            type: 'POST',
